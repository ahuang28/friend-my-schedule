...
def main():
    ...
    if len(sys.argv) != 3:
        print("Usage: python match.py <reference_id> <json string> ")
        sys.exit(1)
    ...
    reference_index = next(i for i, i_d in enumerate(data) if i_d["name"] == reference_id)
    
    corpus = [' '.join(i_d['classes'] + i_d['interests']) for i_d in data]
    ...
    return json.dumps(cosine_similarity_list, indent=4)
